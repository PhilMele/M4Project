...
class Rate (models.Model):
    ...
    user = models.ForeignKey(
        ...
        null=True,
        blank=True)
    parking_name = models.ForeignKey(
        Parking,
        on_delete=models.SET_NULL,
        null=True,
        blank=True
    ...
    timestamp_leave = models.DateTimeField(
        auto_now_add=True,
        null=True,
        blank=True)

    def clean(self):
        # checks if rate field value is not already assigned to another
        # rate object with same parkind_id
        # this is to avoid parking manager creating different rates
        # for the same hour range
        if Rate.objects.filter(
            parking_name=self.parking_name,
            hour_range=self.hour_range
            ).exclude(
                id=self.id
                ).exclude(parking_name__isnull=True).exists():
            raise ValidationError(
                "An hour range with this value"
                "already exists for the selected parking.")
...
class IllegalParking(models.Model):
    ...
    def __str__(self):
        ...
        # that handles potential None values (if an FK is deleted)
        parking_name_str = (
